...
void from_json(const Json &json, InputData &inputData) {
    ...
    bottomRight.color.r = json.at("Corners").at("BottomRight").at("Color").at("R");
    bottomRight.color.g = json.at("Corners").at("BottomRight").at("Color").at("G");
    bottomRight.color.b = json.at("Corners").at("BottomRight").at("Color").at("B");
    bottomRight.color.a = json.at("Corners").at("BottomRight").at("Color").at("A");
...
namespace Option {
    ...
    namespace Program {
        auto name() {
            return "curve-my-corners";
        }

        auto info() {
            return "Generates colored rectangles with curved corners";
        }
    }
    ...
    namespace Help {
        ...
        auto value() {
            return cxxopts::value<bool>()->default_value("false");
        }
    }
    ...
    namespace Input {
        ...
        auto info() {
            return "specify input json file path";
        }
...
int main(const int argc, const char *const *const argv) {
    ...
    try {
        ...
        optionParser.add_options()
            (Help::codes(), Help::info(), Help::value())
        ...
        std::ifstream in(options[Input::code()].as<std::string>(), std::ios_base::in | std::ios_base::binary);
        if(!in) {
        ...
        renderTexture.create(
            static_cast<unsigned int>(inputData.width),
        ...
        sf::CircleShape topLeftCurve(
            topLeftData.radius,
            topLeftData.vertices
        );
        ...
        bottomLeftTriangle[0].position = {0.0F, inputData.height - bottomLeftData.radius};
        ...
        sf::CircleShape bottomLeftCurve(
            bottomLeftData.radius,
            bottomLeftData.vertices
        );

        bottomLeftCurve.setFillColor(sf::Color::Blue);
        bottomLeftCurve.move(
            0.0F,
            inputData.height - bottomLeftCurve.getGlobalBounds().height
        );
        ...
        bottomRightTriangle[0].position = {inputData.width - bottomRightData.radius, inputData.height};
        ...
        bottomRightTriangle[2].position = {inputData.width, inputData.height - bottomRightData.radius};
        bottomRightTriangle[2].color = bottomRightData.color;


        sf::CircleShape bottomRightCurve(
            bottomRightData.radius,
        ...
        topRightTriangle[2].position = {inputData.width - topRightData.radius, 0.0F};
        topRightTriangle[2].color = topRightData.color;

        sf::CircleShape topRightCurve(
            topRightData.radius,
            topRightData.vertices
        );

        topRightCurve.setFillColor(sf::Color::Blue);
        topRightCurve.move(
            inputData.width - topRightCurve.getGlobalBounds().width,
            0.0F
        );
